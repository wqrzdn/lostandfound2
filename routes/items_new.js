...
module.exports = function(upload) {
  // Main browse route does NOT require authentication
  // Get all items (feed) - Public access
  router.get('/', async (req, res) => {
    try {
      ...
      if (date) filter.date = { $gte: new Date(date), $lte: new Date(date + 'T23:59:59') };
      ...
      console.log(`Found ${items.length} items matching the filters`);
      
      // Render the items index page
